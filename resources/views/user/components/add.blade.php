...
@php
    $formTitle = !empty($resep) ? 'Edit' : 'Tulis'
@endphp
    ...
    <div class="row justify-content-center">
        <div class="col-md-6">
            <div class="card border-0">
                <div class="card-body">
                    <h2>{{ $formTitle }} Resepmu...</h2>
                </div>
                <div class="card-body">
                @include('user.partials.flash', ['$errors' => $errors])
                    @if (!empty($resep))
                        {!! Form::model($resep, ['url' => ['/home', $resep->id], 'method' => 'PUT', 'files' =>'true', 'enctype' => 'multipart/form-data']) !!}
                        {!! Form::open(array('url' => '/home', 'files' =>'true', 'enctype' => 'multipart/form-data')) !!}
                        {!! Form::hidden('id') !!}
                    @else
                    {!! Form::model($resep, ['url' => ['/home'],'method' => 'POST', 'files' =>'true', 'enctype' => 'multipart/form-data']) !!}
                    {!! Form::open(array('url' => '/home', 'files' =>true, 'enctype' => 'multipart/form-data')) !!}
                    @endif
                        <div class="mb-3">
                        <div class="form-group">
                            {!! Form::label('judul', 'Judul') !!}
                            {!! Form::text('judul', null, ['class' => 'form-control', 'placeholder' => '']) !!}
                        </div>
                        <div class="form-group">
                        ...
                        <br>
                        <div class="mb-3">
                        {!! Form::label('foto','Upload Foto Masakan') !!}
                        <!-- untuk create -->
                        <img class="img-preview img-fluid mb-3 col-sm-5" style="display: block;" id="pic">
                        {!! Form::file('foto', ['id'=>'foto', 'class' => 'form-control', 'files' =>'true', 'enctype' => 'multipart/form-data',
                                                'oninput'=>'pic.src=window.URL.createObjectURL(this.files[0])', 'placeholder' => '']) !!}
                        </div>
                        </div>
                        </div>
                        <div class="form-footer pt-2">
                            <button type="submit" class="btn btn-primary btn-default"style="display: block;width:580px;background-color:#547794;margin-left:18px;">Terbitkan Resep</button>
                        </div>
                    {!! Form::close() !!}
                </div>
            </div>
